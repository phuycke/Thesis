...
dataset = read.table("Data_Thesis_EXP1_edited.txt", fill = T, header = T, sep = "\t",  na.strings = "")
head(dataset)
...
print(ggplot(standardised_rt, aes(StandardisedPrice, Mean)) + geom_point() + geom_smooth(method = "loess", color = "red")
      ...
      + labs(title = "Mean reaction time (in ms) for each standardized price", subtitle = "Experiment 1")
...
print(ggplot(centered_rt, aes(CenteredPrice, Mean)) + geom_point() + geom_smooth(method = "loess", color = "red")
      ...
      + scale_y_continuous(name="Mean reaction time (in ms)")
      + labs(title = "Mean reaction time (in ms) for each centered price", subtitle = "Experiment 1")
      + theme(plot.title = element_text(hjust = 0.5))
      + theme(plot.subtitle = element_text(hjust = 0.5))
      + theme(axis.text.x=element_text(angle=90,hjust=1)))
...
legend(4.65, 590, legend=c("predicted by linear fit", "predicted by quadratic fit"),
...
legend(1.2, 590, legend=c("predicted by linear fit", "predicted by quadratic fit"),
       col=c("firebrick3", "chartreuse4"), lty=1:2, cex=0.8)
...
print(ggplot(tmp, aes(x = Comparison, y = Estimate, ymin = lwr, ymax = upr)) 
  ...
  + theme_bw()
  + labs(title = "Coefficient estimates for each (categorical) price level", subtitle = "Experiment 1")
  + theme(plot.title = element_text(hjust = 0.5))
  + theme(plot.subtitle = element_text(hjust = 0.5)))
...
round(mean(extreme_zero$RT_in_ms))
round(sd(extreme_zero$RT_in_ms))
